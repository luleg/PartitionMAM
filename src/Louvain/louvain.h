...
class Louvain {
 public:
  ...
  void init_partition(TStr filename_part);
  ...
  void display_partition(TStr OutFile = "");

  // generates the binary graph of communities as computed by one_level
  // Graph partition2graph_binary();
  Graph partition2graph_binary(bool blocked,bool first=false, int kmin = 0);

  // compute communities of the graph for one level
  // return true if some nodes have been moved
  bool one_level(bool blocked, bool verbose = false);
};
